...
public class TrappingRainWater {
    ...
    public int maxArea(int[] height) {
        ...
        for(int i = 0; i < height.length; i++) {
            for(int j = i+1; j < height.length; j++) {
                ...
                System.out.println(sum + " " + height[i] + " " + height[j] );
                maxWater = Math.max(maxWater, sum);
            }
        }
    ...
    public int trap(int[] height) {
        int water = 0;
        int[][] matrix = new int[4][height.length];
        int maxLeftHeight   = 0;
        int maxRightHeight  = 0;

        for (int i = 0; i < height.length; i++) {
            matrix[0][i] = maxLeftHeight;
            maxLeftHeight = Math.max(height[i], maxLeftHeight);
        }

        for (int i = height.length -1; i >= 0; i--) {
            matrix[1][i] = maxRightHeight;
            maxRightHeight = Math.max(height[i], maxRightHeight);
        }
        ...
        for (int i = 0; i < height.length; i++) {
            matrix[3][i] = matrix[2][i] - height[i];
            water += Math.max((matrix[2][i] - height[i]), 0);
        }
